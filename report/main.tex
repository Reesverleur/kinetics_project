...
mechanistically useful if dilution does not fundamentally change the controlling
...
transport resistance, kinetics becomes increasingly important at higher
temperature and pressure, and gas-phase diffusion becomes more relevant as total
pressure decreases. These results suggest that hydrogen dilution does not merely
reduce the overall hydrogen availability; it can also shift the balance among
gas transport, shell transport, and kinetics. Consequently, the use of dilute
\ce{H2}/Ar mixtures in future experiments is most defensible when the operating
conditions remain within the same limiting regime as the pure-hydrogen case of
interest.
\end{abstract}

\section{Introduction}

Cerium hydriding is governed by coupled transport and reaction processes. A
hydrogen molecule must first reach the particle through the surrounding gas,
then traverse any hydride layer that has already formed, and finally react at
the moving Ce/\ce{CeH2} interface. The relative importance of these steps
determines both the interpretation of measured behavior and the extent to which
laboratory conditions can be varied without changing the underlying physics.

The original proposal for this study focused on a characteristic hydriding time
for a hot liquid cerium droplet and asked whether the process was controlled
primarily by gas-phase transport or by interfacial reaction. As the modeling
effort developed, two issues became clear. First, a defensible time-to-conversion
prediction would require thermal feedback from the exothermic reaction, since
temperature changes would alter both transport properties and the kinetics law.
...
``transport-limited'' category. The present report therefore addresses a refined
question: under fixed thermodynamic conditions, is cerium hydriding controlled
...
This refinement is not merely a matter of modeling convenience. In planned
experimental work, using hydrogen diluted in argon would offer a substantial
safety advantage relative to pure hydrogen. However, diluted mixtures are only
scientifically useful if they probe the same limiting mechanism as the
...
The objective of this report is therefore to quantify how the dominant
...
Sarussi et al.\cite{Sarussi1993}
...
$a$ surrounded by an \ce{H2}/Ar gas mixture. The particle outer radius is $b$,
and in the baseline calculations used here the hydride shell thickness is
prescribed so that $b = a + \delta$. At a given operating point, hydrogen
...
The interfacial rate is written in terms of a front speed law,
\begin{equation}
\dot{n}_{k} =
...
diffusion, shell diffusion, and kinetics rather than grouping them into a
single undifferentiated transport limitation.
...
condition of interest, then dilution may provide a safer route to
mechanistically relevant experiments. If, however, dilution shifts the system
into a different controlling regime, then the resulting data will reflect a
...
problem. Most importantly, the interfacial kinetics law is a placeholder based
...
model of absolute hydriding rates.
...
for future research, but only when the experimental conditions are chosen so
that the same limiting regime is preserved.
