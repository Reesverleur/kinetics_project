...
\titlespacing*{\section}{0pt}{1.6ex plus 0.4ex minus 0.2ex}{0.7ex plus 0.2ex}
\titlespacing*{\subsection}{0pt}{1.2ex plus 0.3ex minus 0.2ex}{0.5ex plus 0.2ex}
\setlist{nosep,leftmargin=1.6em}

\title{Effect of Hydrogen Dilution on the Limiting Mechanism for Cerium Hydriding}
\author{R. Verleur\thanks{Code availability: \url{https://github.com/rverleur/kinetics_project}.}}
\date{}
...
\begin{abstract}
This work examines whether diluting hydrogen with argon changes the limiting
mechanism for cerium hydriding. A quasi-steady isothermal spherical model is
used in which hydrogen uptake is controlled by three serial processes:
gas-phase transport through \ce{H2}/Ar, diffusion through a hydride shell, and
interfacial reaction at the Ce/\ce{CeH2} front. The model is used to classify
the dominant bottleneck over temperature, total pressure, hydrogen mole
fraction, and particle size. For the baseline conditions studied here, shell
diffusion is commonly the dominant transport resistance, kinetics becomes more
important as temperature and pressure increase, and gas-phase diffusion matters
mainly at very low hydrogen content. Dilute \ce{H2}/Ar mixtures are therefore
most useful when the operating point remains in the same limiting regime as the
higher-hydrogen condition of interest.
\end{abstract}

\section{Introduction}

The hydriding reaction of cerium is governed by coupled transport and reaction
processes. A hydrogen molecule must first reach the particle through the
surrounding gas, then diffuse through any hydride layer that has already
formed, and finally react at the Ce/\ce{CeH2} interface. The relative rates of
these steps determine the limiting behavior and define the extent to which
laboratory conditions can be varied without changing the underlying physics.

The original proposal for this project focused on a characteristic hydriding
time for a hot liquid cerium droplet and asked whether the process was
controlled primarily by gas-phase transport or by interfacial reaction. As
work on the project progressed, two major difficulties motivated a change in
focus. First, a defensible ``characteristic hydriding time'' prediction would
require a fully coupled model for heat transfer at the surface. Second, once a
hydride layer is included, diffusion through that layer becomes a separate
candidate bottleneck and cannot be folded into a generic
``transport-limited'' category. These issues were compounded by the inability
to find an accurate kinetic model for the hydriding reaction of molten cerium.
The present report therefore addresses a related question:
\textbf{\textit{Under fixed thermodynamic conditions, is cerium hydriding controlled
...
safety advantage relative to pure hydrogen. However, diluting the mixture risks
changing the limiting mechanism relative to the conditions of real interest. If
dilution leaves the controlling regime unchanged, then reduced-\ce{H2}
experiments may still provide mechanistically relevant data. If dilution shifts
the system from shell-diffusion control to gas-diffusion control, or from
transport control to kinetic control, then the results would have to be
interpreted much more cautiously.
...
Sarussi et al.\cite{Sarussi1993} despite its limited applicability to molten
cerium.
...
$a$, surrounded by a layer of cerium hydride of thickness $\delta$, which is in
turn surrounded by a \ce{H2}/Ar gas mixture. The radius of the solid phase is
defined as $b$ so that $b = a + \delta$. At a given operating point, hydrogen
...
diffusion, shell diffusion, and kinetics, thereby helping ensure that any
reduced-hydrogen experiments remain in the same limiting regime.
...
appears only in an extremely narrow low-\ce{H2} region, which indicates that, under most
conditions the external gas supply is faster than transport through the shell.
Above 1690 K, \ce{CeH2} melts, and, consistent with Regele et
al.\cite{Regele2023}, the diffusion coefficient is increased by an order of
magnitude. This produces the abrupt change in controlling mechanism at that
temperature.
\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\textwidth]{limitation_maps_by_size.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.96\textwidth]{limitation_maps_by_pressure.png}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.95\textwidth]{radial_profiles_by_yinf.png}
...
condition of interest, then dilution may provide a safer route to performing
experiments that are still mechanistically relevant. If, however, dilution shifts the system
...
problem. Most importantly, the interfacial kinetics law is likely inaccurate
because it is based on solid-cerium data.\cite{Sarussi1993} The present results should therefore be
interpreted primarily as a mechanistic regime study rather than as a predictive
model of absolute hydriding rates. If a more accurate kinetic mechanism for
liquid cerium hydriding becomes available, it can be substituted into the
present framework relatively easily, and the resulting predictions should be
much more reliable.
...
that the same limiting regime is preserved and the results are interpreted with
care.
